...
exports.DefenderPlayer = void 0;
var Position_1 = require("./Position");
var Food_1 = require("./Food");
var DefenderPlayer = /** @class */ (function () {
    function DefenderPlayer(position) {
    ...
    DefenderPlayer.prototype.tick = function (barbarianAssault) {
    ...
    DefenderPlayer.prototype.findPath = function (barbarianAssault, destination) {
        for (var i = 0; i < barbarianAssault.map.width * barbarianAssault.map.height; i++) {
        ...
        this.pathQueueIndex = 0;
        var pathQueueEnd = 0;
        this.pathQueuePositions[this.pathQueueIndex] = new Position_1.Position(this.position.x, this.position.y);
        this.pathQueueIndex++;
        var currentPosition;
        var foundDestination = false;
        while (this.pathQueueIndex !== pathQueueEnd) {
            currentPosition = new Position_1.Position(this.pathQueuePositions[this.pathQueueIndex].x, this.pathQueuePositions[this.pathQueueIndex].y);
            if (currentPosition.equals(destination)) {
                foundDestination = true;
                break;
            }
            var newDistance = this.shortestDistances[currentPosition.x + currentPosition.y * barbarianAssault.map.width] + 1;
            var index = currentPosition.x - 1 + currentPosition.y * barbarianAssault.map.width;
            if (currentPosition.x > 0 && this.waypoints[index] === 0 && (barbarianAssault.map.map[index] & 19136776) === 0) {
                this.pathQueuePositions[pathQueueEnd] = new Position_1.Position(currentPosition.x - 1, currentPosition.y);
                pathQueueEnd++;
            ...
            if (currentPosition.y > 0 && this.waypoints[index] === 0 && (barbarianAssault.map.map[index] & 19136770) === 0) {
                this.pathQueuePositions[pathQueueEnd] = new Position_1.Position(currentPosition.x, currentPosition.y - 1);
                pathQueueEnd++;
            ...
            if (currentPosition.x > 0 && currentPosition.y > 0 && this.waypoints[index] === 0 && (barbarianAssault.map.map[index] & 19136782) === 0
                ...
                this.pathQueuePositions[pathQueueEnd] = new Position_1.Position(currentPosition.x - 1, currentPosition.y - 1);
                pathQueueEnd++;
        ...
        if (!foundDestination) {
            var bestDistanceStart = 0x7FFFFFFF;
            var bestDistanceEnd = 0x7FFFFFFF;
            var deviation = 10;
            for (var x = destination.x - deviation; x <= destination.x + deviation; x++) {
                for (var y = destination.y - deviation; y <= destination.y + deviation; y++) {
                    if (x >= 0 && y >= 0 && x < barbarianAssault.map.width && y < barbarianAssault.map.height) {
                        var distanceStart = this.shortestDistances[x + y * barbarianAssault.map.width];
                        if (distanceStart < 100) {
                            var distanceEnd = Math.pow(Math.max(destination.x - x), 2) + Math.pow(Math.max(destination.y - y), 2);
                            if (distanceEnd < bestDistanceEnd || (distanceEnd === bestDistanceEnd && distanceStart < bestDistanceStart)) {
                                ...
                                currentPosition = new Position_1.Position(x, y);
                                foundDestination = true;
                            }
                        }
                    }
        ...
        while (!currentPosition.equals(this.position)) {
            var waypoint = this.waypoints[currentPosition.x + currentPosition.y * barbarianAssault.map.width];
            this.pathQueuePositions[this.pathQueueIndex] = new Position_1.Position(currentPosition.x, currentPosition.y);
            this.pathQueueIndex++;
            if ((waypoint & 2) !== 0) {
                currentPosition.x++;
            }
    ...
    DefenderPlayer.prototype.draw = function (renderer) {
        if (this.position.x >= 0) {
            renderer.setDrawColor(240, 240, 240, 200);
            renderer.fill(this.position.x, this.position.y);
        }
    };
    DefenderPlayer.prototype.dropFood = function (barbarianAssault, foodType) {
        if ((this.foodInInventory[foodType] > 0 || barbarianAssault.infiniteFood) && this.repairTicksRemaining === 0) {
            ...
            barbarianAssault.map.addFood(new Food_1.Food(new Position_1.Position(this.position.x, this.position.y), foodType, foodType === barbarianAssault.defenderFoodCall));
        }
    };
    DefenderPlayer.prototype.startRepairing = function (barbarianAssault) {
    ...
    DefenderPlayer.prototype.repair = function (barbarianAssault) {
    ...
    DefenderPlayer.prototype.repairTrap = function (barbarianAssault) {
    ...
    DefenderPlayer.prototype.pickUpFood = function (barbarianAssault) {
        if (this.foodBeingPickedUp === null) {
            return;
        }
        var foodZone = barbarianAssault.map.getFoodZone(this.position.x >>> 3, this.position.y >>> 3);
        for (var i = 0; i < foodZone.foodList.length; i++) {
            var food = foodZone[i];
    ...
    DefenderPlayer.prototype.pickUpLogs = function (barbarianAssault) {
        if (!this.isPickingUpLogs) {
            return;
        }
    ...
    DefenderPlayer.prototype.move = function () {
    ...
    DefenderPlayer.prototype.takeSteps = function (steps) {
        var stepsTaken = 0;
    ...
    DefenderPlayer.prototype.isInRepairRange = function (position) {
        return Math.abs(this.position.x - position.x) + Math.abs(this.position.y - position.y) <= 1;
    };
    return DefenderPlayer;
}());
exports.DefenderPlayer = DefenderPlayer;
